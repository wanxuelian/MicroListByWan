...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate
{
    ...
    if (self) {
        ...
        currentExtendSection = -1;//默认－1没有展开
...
-(void)showChooseListViewInSection:(NSInteger)section choosedIndex:(NSInteger)index
{
    if (!self.mTableView) {
        ...
        self.mTableBaseView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x + rect.size.width * 2 / 3, self.frame.origin.y + self.frame.size.height + 55, self.frame.size.width, self.mSuperView.frame.size.height - self.frame.origin.y - self.frame.size.height)];
        ...
        self.mTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height + 55, self.frame.size.width, 240) style:UITableViewStylePlain];
